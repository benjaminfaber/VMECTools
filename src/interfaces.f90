...
contains
  ...
  !****************************************************************************
  subroutine pest2vmec_c_interface(vmec_file,nx1,nx2,nx3,nfpi,x1,grid_type) bind(C,name='pest2vmec_c_interface')
    character(c_char), dimension(:), intent(in) :: vmec_file
    integer(c_int), intent(in) :: nx1, nx2, nx3
    real(c_double), value, intent(in) :: nfpi
    real(c_double), dimension(nx1) :: x1 
    character(c_char), dimension(:), intent(in) :: grid_type
...
print *, nx1, nx2, nx3
    vmec_string = ""
    do i = 1,size(vmec_file)
      vmec_string = vmec_string//vmec_file(i)
    end do

    grid_string = ""
    do i = 1,size(grid_type)
      grid_string = grid_string//grid_type(i)
    end do

    pest = create_PEST_Obj(vmec_string,x1,nx2,nx3)
    if (grid_string .eq. "tok") then 
      norm_type = 'major_R'
    else
      norm_type = 'minor_r'
      x3_coord = 'theta'
    ...
    call set_normalizations(pest,grid_string) 
    
  end subroutine

  subroutine get_pest_data_c_interface(data_name,n_dims,x1,x2,x3,data_size,c_data) bind(C,name="get_pest_data_c_interface")
    integer(c_int), intent(in) :: n_dims, data_size, x1, x2, x3
    character(c_char), dimension(:), intent(in) :: data_name
    ...
    do idx1=1,size(data_name)
      data_string = data_string//data_name(idx1)
    end do
    ...
    select case(n_dims)
      case(1)
        ...
        call get_PEST_data(pest,x1,x2,data_string,line_data)
      ...
      case(2)
        ...
        call get_PEST_data(pest,x1,data_string,surf_data)
        do idx3 = pest%ix31,pest%ix32
      ...
      case(3)
        ...
        call get_PEST_data(pest,data_string,vol_data)
